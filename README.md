...
exports **registerAnimation(fnHandler)** - register animation callback handler
exports **unregisterAnimation(fnHandler)** - unregister animation callback handler

# Example
```
import { registerAnimation } from "./../lib/js-animation/globals/animation.js"

/**
 * @param {number} iAnimationTime
 * @param {number} iInterval
function onAnimate(iAnimationTime, iInterval) {
	console.log("Animate", iAnimationTime, iInterval);
}

registerAnimation(onAnimate);
```
